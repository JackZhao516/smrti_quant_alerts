...
PWD = os.path.join(Config.PROJECT_DIR, "smrti_quant_alerts")
...
def read_exclude_coins_from_file() -> Set[str]:
    ...
    """
    exclude_coins = set()
    if os.path.exists(os.path.join(PWD, "stable_coins.json")):
        with open(os.path.join(PWD, "stable_coins.json")) as f:
            exclude_coins.update(json.load(f))

    if os.path.exists(os.path.join(PWD, "exclude_coins.json")):
        with open(os.path.join(PWD, "exclude_coins.json"), "r") as f:
            exclude_coins.update(json.load(f))
